import React from 'react';
import { Container } from 'react-bootstrap';

const Contnent = () => {
    return (
        <div>
              <Container>
                
                <h1 className='text-center p-5 brown-color'>About us</h1>
                <p  className='text-center p-4'>
                Elite Realty is a Business Consulting Firm, offering consultancy services to investors who intend setting up new businesses in the field of wholesale, retail, distribution and service- provider’s landscape. Our team of professionals who bring in years of expertise in various business verticals, assist clients to set-up new business ventures by way of owning franchise of prominent business brands or set-up independent business from ground up.
                </p>
                <p className='text-center p-4'>
                We provide clients with time tested proven business concepts and models. Our team helps you to choose a business category or a franchise brand that aligns best with you passion and personality. Everyone wants to go to work and do something that they love. If you are going to invest in a business, it should be in something you are passionate about. We offer multiple pre-screened and qualified list of choices in many business categories. This enables our clients to steer away from business concepts or franchise brands that appear good on paper but facts relate otherwise.
                </p>
                <p className='text-center p-4'>
                Before arriving at a decision to set up any business in a particular region; we conduct region specific surveys and conduct due diligence studies to assess the viability of that business in that particular region. This enables us to stratagies, to take on any present or future competition for business. This helps in increasing the of success rate of the new business. Our team with its deep insights, ensures that your business escapes all the mistakes you would make while figuring out a business on your own.
                </p>
                <p className='text-center p-4'>
                We negotiate on your behalf with franchising brands for the right terms & conditions. We bargain for various concessions at the time of buying a brands franchisee. We insure that you put your money on the right things, not just agree to any undue expectation of the franchising brand.
                </p>
                <p className='text-center p-4'> 
                Our consultancy and supervision is readily available for our clients, who intend to set up a stand-alone (independent) business. We work on a host of different business categorise which already stand successful in the market. Our experts guide our clients in merchandising of standardised products and systems.  
                </p>
                      </Container>
        </div>
    );
}

export default Contnent;